...
import { loadSettings, markTutorialSeen } from './settings.js'
...
export class Level1Scene extends BaseLevelScene {
  ...
  createEnemies() {
    this.registerEnemy(new SoundNinja(this, 15 * 64, 18 * 64))
    this.registerEnemy(new SoundNinja(this, 25 * 64, 15 * 64))
    this.registerEnemy(new SoundNinja(this, 15 * 64, 14 * 64))
  }
  ...
  startTutorialIfNeeded() {
    ...
    const steps = [
      { delay: 400, text: 'Move with WASD or Arrow Keys.' },
      { delay: 3200, text: 'Use J (Punch) and K (Kick) for close combat.' },
      { delay: 6200, text: 'Use L and U for character skills.' },
      { delay: 9200, text: 'Collect air bubbles to refill O2.' },
      { delay: 12200, text: 'Defeat all enemies to clear the level.' }
    ]

    this.tutorialTexts = []
    steps.forEach(step => {
      this.time.delayedCall(step.delay, () => this.showTutorialStep(step.text))
    })

    this.time.delayedCall(15500, () => {
      markTutorialSeen()
      this.clearTutorialSteps()
    })
  }

  showTutorialStep(text) {
    if (!this.cameras || !this.cameras.main) return
    const cam = this.cameras.main
    this.clearTutorialSteps()

    const panel = this.add.rectangle(cam.scrollX + cam.width / 2, cam.scrollY + 90, 640, 54, 0x000000, 0.55)
      .setStrokeStyle(2, 0x87ceeb, 0.9)
      .setDepth(4000)
      .setScrollFactor(0)
    const label = this.add.text(cam.scrollX + cam.width / 2, cam.scrollY + 90, text, {
    ...
    this.tutorialTexts = [panel, label]
    this.tweens.add({
      targets: this.tutorialTexts,
      alpha: 0,
      duration: 550,
      delay: 2200,
      onComplete: () => this.clearTutorialSteps()
    })
  }

  clearTutorialSteps() {
    if (!this.tutorialTexts) return
    this.tutorialTexts.forEach(item => item && item.destroy && item.destroy())
    this.tutorialTexts = []
  }
  ...
  createEnvironmentalStoryElements() {
    ...
    this.add.text(12 * 64, 14.5 * 64 - 25, '?', {
      fontFamily: 'RetroPixel, monospace',
      fontSize: '20px',
      fill: '#ffdd88'
