...
export class KakashiPlayer extends Phaser.Physics.Arcade.Sprite {
  constructor(scene, x, y) {
    ...
    this.hurtingDuration = kakashiConfig.hurtingDuration.value // 
    this.invulnerableTime = kakashiConfig.invulnerableTime.value // 
    ...
    this.lastChidoriTime = 0 // 
    this.lastSharinganTime = 0 // 
  ...
  clearStaleActionStates() {
    ...
    if (lockedByAction && currentAnim === 'kakashi_idle_anim' && Math.abs(this.body.velocity.x) < 1) {
      this.isPunching = false
      this.isKicking = false
      this.isChidori = false
      this.isSharingan = false
