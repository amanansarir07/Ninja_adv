...
export class ToadSummon extends Phaser.Physics.Arcade.Sprite {
  constructor(scene, x, y) {
    ...
    this.summonDuration = 15000 // 15秒后消失
    this.attackCooldown = 3000 // 3秒攻击冷却
  ...
  updateAI() {
    ...
    if (shouldDodge && this.canJump()) {
      ...
      // 主动寻敌模式 - 积极追击
      this.performAggressiveHunt(nearestEnemy, nearestDistance)
  ...
  // 积极寻敌行为 - 主动跳跃追击
  performAggressiveHunt(enemy, distance) {
    this.aiState = 'hunt'
    this.target = enemy
