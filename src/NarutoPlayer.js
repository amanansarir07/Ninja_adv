...
export class NarutoPlayer extends Phaser.Physics.Arcade.Sprite {
  ...
  useRasengan() {
    ...
    this.scheduleActionFailsafe('isRasengan', 1200, () => {
    ...
    this.once(Phaser.Animations.Events.ANIMATION_COMPLETE, (animation) => {
      if (animation.key === "naruto_rasengan_anim") {
        this.clearActionFailsafe('isRasengan')
        // 螺旋丸冲击
  ...
  useSummoning() {
    ...
    this.resetOriginAndOffset()
    this.scheduleActionFailsafe('isSummoning', 1300)
    
    this.once(Phaser.Animations.Events.ANIMATION_COMPLETE, (animation) => {
      if (animation.key === "naruto_summoning_anim") {
        this.clearActionFailsafe('isSummoning')
  ...
  clearStaleActionStates() {
    ...
    const lockedByAction = this.isPunching || this.isKicking || this.isRasengan || this.isSummoning
    if (lockedByAction && currentAnim === 'naruto_idle_anim' && Math.abs(this.body.velocity.x) < 1) {
      this.isPunching = false
      this.isKicking = false
      this.isRasengan = false
      this.isSummoning = false
