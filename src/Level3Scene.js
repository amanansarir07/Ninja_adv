...
export class Level3Scene extends BaseLevelScene {
  ...
  createEnemies() {
    this.registerEnemy(new SwiftNinja(this, 11 * 64, 17 * 64))
    this.registerEnemy(new EliteNinja(this, 19 * 64, 18 * 64))
    this.registerEnemy(new SoundNinja(this, 21 * 64, 13 * 64))
    this.registerEnemy(new SwiftNinja(this, 15 * 64, 10 * 64))
    this.registerEnemy(new EliteNinja(this, 25 * 64, 9 * 64))

    this.templeBoss = new BossEnemy(this, 34 * 64, 18 * 64)
    this.registerEnemy(this.templeBoss, { isBoss: true })
  }

  checkBossArenaIntro() {
    if (this.isBossIntroActive) return
    if (!this.player || !this.templeBoss || this.templeBoss.isDead) return
    if (this.player.x < 29 * 64) return
    ...
    this.triggerBossIntro(
      ...
      [
        ...
        'The temple protector awakens.',
        'Prepare for battle!'
      ],
      {
        introKey: 'level3_temple_guardian_intro',
        bossTargets: [this.templeBoss],
        bossHUDTitle: 'TEMPLE GUARDIAN',
        roarSoundKey: 'ninja_hurt_sound'
      }
    )
  }
