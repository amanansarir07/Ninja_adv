...
export class SasukePlayer extends Phaser.Physics.Arcade.Sprite {
  constructor(scene, x, y) {
    ...
    this.hurtingDuration = sasukeConfig.hurtingDuration.value
  ...
  handleAttacks(jKey, kKey, lKey, uKey) {
    ...
    if (Phaser.Input.Keyboard.JustDown(jKey) && !this.isPunching) {
      ...
      this.scheduleActionFailsafe('isPunching', 500, () => this.currentMeleeTargets.clear())
      this.once(Phaser.Animations.Events.ANIMATION_COMPLETE, (animation) => {
        if (animation.key === "sasuke_punch_anim") {
          this.clearActionFailsafe('isPunching')
          this.isPunching = false
          this.currentMeleeTargets.clear()
        }
      })
    ...
    if (Phaser.Input.Keyboard.JustDown(kKey) && !this.isKicking) {
      ...
      this.resetOriginAndOffset()
      this.kickSound.play()
      this.scheduleActionFailsafe('isKicking', 550, () => this.currentMeleeTargets.clear())
      this.once(Phaser.Animations.Events.ANIMATION_COMPLETE, (animation) => {
        if (animation.key === "sasuke_kick_anim") {
          this.clearActionFailsafe('isKicking')
          this.isKicking = false
          this.currentMeleeTargets.clear()
        }
      })
  ...
  useChidori() {
    ...
    this.resetOriginAndOffset()
    this.scheduleActionFailsafe('isChidori', 1200)
    
    this.once(Phaser.Animations.Events.ANIMATION_COMPLETE, (animation) => {
      if (animation.key === "sasuke_chidori_anim") {
        this.clearActionFailsafe('isChidori')
  ...
  useSharingan() {
    ...
    this.scheduleActionFailsafe('isSharingan', skillDuration + 300, resetSharinganState)
      ...
      if (animation.key === "sasuke_sharingan_summon_anim") {
          ...
          if (this.scene.tweens && this.sharinganEffect) {
            ...
            this.scene.tweens.add({
              ...
              onComplete: () => {
                ...
                this.clearActionFailsafe('isSharingan')
                resetSharinganState()
              }
            })
          }
        ...
        this.scene.time.delayedCall(1000, () => {
          if (nearestEnemy && nearestEnemy.active && !nearestEnemy.isDead) {
            ...
            nearestEnemy.once(Phaser.Animations.Events.ANIMATION_COMPLETE, (animation) => {
  ...
  scheduleActionFailsafe(flagKey, durationMs, onReset) {
    this.clearActionFailsafe(flagKey)
    this.actionFailsafes[flagKey] = this.scene.time.delayedCall(durationMs, () => {
      if (!this.active) return
      if (this[flagKey]) {
        this[flagKey] = false
        if (onReset) onReset()
      }
      delete this.actionFailsafes[flagKey]
    })
  }

  clearActionFailsafe(flagKey) {
    const timer = this.actionFailsafes[flagKey]
    if (timer) {
      timer.remove()
      delete this.actionFailsafes[flagKey]
    }
  ...
  clearStaleActionStates() {
    if (this.isDead || this.isHurting) return
    const currentAnim = this.anims && this.anims.currentAnim ? this.anims.currentAnim.key : ''
    const lockedByAction = this.isPunching || this.isKicking || this.isChidori || this.isSharingan
    if (lockedByAction && currentAnim === 'sasuke_idle_anim' && Math.abs(this.body.velocity.x) < 1) {
      this.isPunching = false
      this.isKicking = false
      this.isChidori = false
      this.isSharingan = false
